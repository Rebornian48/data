...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <a href="{{ route('members.index') }}" class="text-sm text-slate-600 hover:text-brand mb-4 inline-flex items-center gap-1">
        ...
        <div class="md:col-span-1">
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                ...
                <div class="p-5">
                    ...
                    @if ($member->nickname)
                        <div class="text-lg text-slate-500 mb-3">"{{ $member->nickname }}"</div>
                    @endif
                    ...
                    <dl class="space-y-2 text-sm">
                        @if ($member->birth_place || $member->birth_date)
                            <div class="flex justify-between">
                                <dt class="text-slate-500">Lahir</dt>
                                <dd class="text-slate-900 text-right">
                                    {{ $member->birth_place }}<br>
                                    <span class="text-xs text-slate-500">{{ $member->birth_date?->format('d M Y') }}</span>
                                </dd>
                            </div>
                        @endif
                        @if ($member->current_age)
                            <div class="flex justify-between">
                                ...
                                <dd class="font-semibold text-slate-900">{{ $member->current_age }} tahun</dd>
                            </div>
                        @endif
                    </dl>
                </div>
        ...
        <div class="md:col-span-2 space-y-6">
            ...
            <div class="grid grid-cols-3 gap-3">
                <div class="bg-white rounded-xl p-4 border border-slate-200 text-center">
                    ...
                    <div class="text-xs text-slate-500 mt-1">Tahun di JKT48</div>
                </div>
                <div class="bg-white rounded-xl p-4 border border-slate-200 text-center">
                    <div class="text-3xl font-bold text-amber-600">{{ $member->totalSenbatsu() }}</div>
                    <div class="text-xs text-slate-500 mt-1">Senbatsu</div>
                </div>
                <div class="bg-white rounded-xl p-4 border border-slate-200 text-center">
                    <div class="text-3xl font-bold text-purple-600">{{ $member->totalCenter() }}</div>
                    <div class="text-xs text-slate-500 mt-1">Center</div>
                </div>
            </div>

            {{-- Timeline --}}
            <div class="bg-white rounded-xl p-6 border border-slate-200">
                <h2 class="text-lg font-bold text-slate-900 mb-4">Timeline Karier</h2>
                <div class="space-y-4">
                    @php
                        $events = [];
                    ...
                    @foreach ($events as $event)
                        <div class="flex gap-4">
                            ...
                            <div class="pb-4 flex-1">
                                <div class="text-xs text-slate-500">{{ $event['date']->format('d M Y') }}</div>
                                <div class="text-slate-900 font-medium">{{ $event['label'] }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            @if ($member->singles->count() > 0)
                <div class="bg-white rounded-xl p-6 border border-slate-200">
                    <h2 class="text-lg font-bold text-slate-900 mb-4">Partisipasi Single</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        @foreach ($member->singles as $single)
                            <div class="flex items-center gap-3 p-2 rounded-lg bg-slate-50">
                                <div class="text-xs font-bold text-slate-400 w-10">{{ $single->code }}</div>
                                <div class="flex-1 min-w-0">
                                    <div class="text-sm font-medium text-slate-900 truncate">{{ $single->title }}</div>
                                </div>
                                @if ($single->pivot->role === 'center')
                                    <span class="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-medium">Center</span>
                                @endif
                            </div>
                        @endforeach
                    </div>
            ...
            @if ($member->captains->count() > 0)
                <div class="bg-white rounded-xl p-6 border border-slate-200">
                    <h2 class="text-lg font-bold text-slate-900 mb-4">Riwayat Kapten</h2>
                    <div class="space-y-2">
                        @foreach ($member->captains->sortBy('start_date') as $captain)
                            <div class="flex items-center justify-between p-3 rounded-lg bg-slate-50">
                                <div>
                                    <div class="font-medium text-slate-900">{{ $captain->position }}</div>
